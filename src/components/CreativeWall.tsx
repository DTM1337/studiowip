...
export default function CreativeWall({ initialPosts, uploaderName }: Props) {
  ...
  return (
    <div className="wall-root">
        ...
        <div className="topbar-right">
          <input
            ...
            onChange={(e) => {
              ...
              localStorage.setItem('showandtell_name', e.target.value)
            }}
          />
          <input
            className="field cp"
      ...
      <style>{`
        .wall-root { min-height: 100vh; background: #efefef; overflow: hidden; }
        .topbar {
        ...
        .upload-btn:hover:not(.uploading) { opacity: .82; transform: scale(1.02); }
        .upload-btn.uploading { opacity: .5; cursor: default; }
        .wall-stage {
          display: flex; align-items: center; justify-content: center;
          min-height: 100vh; padding-top: 64px;
        }
        .wall-canvas { position: relative; width: 1400px; height: 1200px; }
        .drop-overlay {
...
function DraggableCard({ post, initX, initY, rotation, width, onMoved, onClick }: CardProps) {
  ...
  return (
    <motion.article
      ...
      initial={{ x: initX, y: initY, rotate: rotation }}
      onDragStart={() => { dragDist.current = 0 }}
      onDrag={(_, info) => {
        dragDist.current = Math.hypot(info.offset.x, info.offset.y)
      }}
      onDragEnd={(_, info) => {
        onMoved(info.offset.x, info.offset.y)
      }}
    ...
    >
      ...
      <style>{`
        ...
        .wall-card:hover {
          box-shadow: 0 18px 52px rgba(0,0,0,.22), 0 2px 8px rgba(0,0,0,.1);
          z-index: 50;
        }
        .card-media {
          position: relative;
